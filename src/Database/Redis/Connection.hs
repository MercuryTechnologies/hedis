...
connectCluster bootstrapConnInfo = do
    ...
    case commandInfos of
        ...
        Right infos -> do
#if MIN_VERSION_resource_pool(0,3,0)
            pool <- newPool (defaultPoolConfig (Cluster.connect infos shardMapVar Nothing (connectTLSParams bootstrapConnInfo) $ connectHooks bootstrapConnInfo) Cluster.disconnect (realToFrac $ connectMaxIdleTime bootstrapConnInfo) (connectMaxConnections bootstrapConnInfo))
#else
            pool <- createPool (Cluster.connect infos shardMapVar Nothing (connectTLSParams bootstrapConnInfo) $ connectHooks bootstrapConnInfo) Cluster.disconnect 1 (connectMaxIdleTime bootstrapConnInfo) (connectMaxConnections bootstrapConnInfo)
#endif
